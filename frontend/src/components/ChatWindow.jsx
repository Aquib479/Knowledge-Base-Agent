...
export default function ChatWindow({ messages, thinking, onAsk, onClear, docCount }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 11, fontFamily: 'DM Mono', color: 'var(--text-3)', marginTop: 2 }}>
      ...
      <div style={{
        ...
        >
          <textarea
            ...
            onKeyDown={onKeyDown}
            placeholder={docCount === 0 ? 'Upload a PDF first...' : 'Ask anything about your documents...'}
            disabled={docCount === 0 || thinking}
            style={{
              flex: 1, background: 'transparent', border: 'none', outline: 'none',
              color: 'var(--text)', fontFamily: 'Geist', fontSize: 14, lineHeight: 1.6,
          ...
          <button
            onClick={submit}
            disabled={!input.trim() || thinking || docCount === 0}
            style={{
              width: 36, height: 36, borderRadius: 8, flexShrink: 0,
              background: input.trim() && !thinking && docCount > 0 ? 'var(--accent)' : 'var(--border)',
              border: 'none', cursor: input.trim() && !thinking && docCount > 0 ? 'pointer' : 'default',
        ...
        <p style={{
          fontSize: 10, fontFamily: 'DM Mono', color: 'var(--text-3)',
...
function ThinkingIndicator() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-start' }}>
      <span style={{ fontSize: 9, fontFamily: 'DM Mono', letterSpacing: 1.5, color: 'var(--text-3)' }}>
        AGENT
      </span>
      <div style={{
        background: 'var(--bg-2)',
...
function EmptyState({ docCount }) {
  ...
  return (
    <div style={{
      ...
      minHeight: '50vh', gap: 32, textAlign: 'center',
    }}>
      <div>
        <p style={{
          fontFamily: 'Instrument Serif', fontSize: 32,
          color: 'var(--text)', fontStyle: 'italic', marginBottom: 8,
          opacity: 0.9,
        }}>
          What do you want to know?
        </p>
        <p style={{ fontSize: 13, fontFamily: 'DM Mono', color: 'var(--text-3)' }}>
          {docCount === 0
