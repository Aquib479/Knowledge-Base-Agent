...
export default function ChatMessage({ message, index }) {
  ...
  return (
    ...
    >
      {/* Role label */}
      <span style={{
        fontSize: 9, fontFamily: 'DM Mono', letterSpacing: 1.5,
        color: isUser ? 'var(--accent-dim)' : isError ? 'var(--red)' : 'var(--text-3)',
      }}>
      ...
      <div style={{
        maxWidth: '78%',
        background: isUser ? 'var(--bg-3)' : isError ? '#1a0e0e' : 'var(--bg-2)',
